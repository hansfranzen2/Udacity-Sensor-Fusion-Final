...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        p_veh = np.ones((4, 1))
        p_veh[0:3] = x[0:3] # Object position in homogeneous vehicle coordinates
        p_sens = self.veh_to_sens*p_veh

        if p_sens[0] > 0: # Prevent division by zero; negative value means it's behind the car, so invisible to the camera
            alpha = np.arctan(p_sens[1]/p_sens[0])
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            # END student code
            ############ 
        # print("New "+sensor.name+" measurement: "+str(self.z))